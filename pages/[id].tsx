...
const fetcherAll = (args: string) => fetch(args).then((res): Promise<ProductInterface[]> => res.json());
const fetcherSingle = (args: string) => fetch(args).then((res): Promise<ProductInterface> => res.json());

const ProductDetail = () => {
  const { query } = useRouter();
  const { id } = query;

  const { data, isValidating } = useSWR(`/api/products/${id}`, fetcherSingle);
  const { data: products } = useSWR('/api/products', fetcherAll);
  ...
  useEffect(() => {
    if (products && data) {
      const relateds = products.filter((el) => el.category === data.category && el._id !== data._id);
          ...
          <div className="">
            <div className="grid xs:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2  gap-5">
              ...
              <div className="flex items-start justify-center flex-col">
                ...
                <span className="text-xs font-supremeLight">Ref: {data?.product_id}</span>
                <p className="w-full my-5 leading-7">{data?.content}</p>
                <div className="flex mb-3 ">
                  <p className="text-zinc-500 text-sm mr-3">Available: {data?.stock}</p>
                ...
                {data && (
                  <div className="xs:mt-5 sm:mt-20 md:mt-5 lg:mt-20 xl:mt-20">
                    ...
                    <button className=" p-2.5 ">
                      <span className="border-b-2 border-zinc-900 pb-1" onClick={() => addToWishList({ ...data })}>
                        Add to wish list
                      </span>
                    </button>
                  </div>
